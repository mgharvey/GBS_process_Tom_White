...
while($entry=<IN>) {
	...
	if($missing < $accept_missing && $obsHet < $accept_hetero && $FIS != -9) {
		...
		$meanLocCov = $locCount / $sampleSize;
		if($meanLocCov > $maxLocCov) {
			$maxLocCov = $meanLocCov;
		}
		if($meanLocCov < $minLocCov) {
			$minLocCov = $meanLocCov;
		}
		print OUTFILE1 "$meanLocCov\n";
		$locs++;
		$totCount += $locCount;
	}
...
$meanCov = $totCount / ($locs * $sampleSize);
print "Locs: $locs\n";
print "Mean cov: $meanCov\n";
print "Max cov: $maxLocCov\n";
print "Min cov: $minLocCov\n";
...
sub calc_genotype {
	...
	my $eq_part2A = $count1 * log(1.0 - $error_rate);
	#my $eq_part2A = (1.0 - ((3*$error_rate)/4)) ** $count1;
	my $eq_part2B = $tot_count * log(0.5);
	my $eq_part2C = $count2 * log(1.0 - $error_rate);
	my $eq_part3A = $count2 * log($error_rate);
	my $eq_part3B = 1;
	my $eq_part3C = $count1 * log($error_rate);
